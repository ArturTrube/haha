    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (path) {
            ...
            case "/changeRole":
                ...
                if (personFacade.find(Long.parseLong(personId)).getLogin().equals(authPerson.getLogin())){
                    request.setAttribute("info", "Вы не можете поменять роль самому себе!");
                    request.getRequestDispatcher("/adminPanel").forward(request, response);
                    break;
                }
                if (personFacade.find(Long.parseLong(personId)).getLogin().equals("admin")){
                    request.setAttribute("info", "Вы не можете поменять роль этому пользователю!");
                    request.getRequestDispatcher("/adminPanel").forward(request, response);
                    break;
